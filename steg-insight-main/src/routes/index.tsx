...
import { formatTND, formatDate, clientById, PIE_COLORS, tooltipStyle } from "@/lib/store";
...
function Dashboard() {
  ...
  const { data: riskScores = [], isLoading: riskScoresLoading } = useRiskScores();
  const { data: k, isLoading: kpisLoading } = useKpis();
  const { data: series = [], isLoading: seriesLoading } = useMonthlySeries();
  const { data: pie = [], isLoading: pieLoading } = useStatusBreakdown();
  ...
  const recentPayments = useMemo(
    () => [...payments].sort((a, b) => b.datePaiement.localeCompare(a.datePaiement)).slice(0, 5),
    [payments],
  );
  ...
  if (clientsLoading || invoicesLoading || paymentsLoading || riskScoresLoading || kpisLoading || seriesLoading || pieLoading) {
    return (
  ...
  const kpis = k ?? { totalFacture: 0, totalPaye: 0, montantImpaye: 0, nbFactures: 0, nbImpayees: 0, nbClients: 0, tauxRecouvrement: 0, tauxImpaye: 0 };
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <div className="animate-slide-up stagger-2">
          <KpiCard
            ...
            icon={TrendingUp}
            tone="success"
            hint={`Taux de recouvrement ${kpis.tauxRecouvrement.toFixed(1)} %`}
            trend={evolutionPaye}
          />
        </div>
        <div className="animate-slide-up stagger-3">
          <KpiCard
            ...
            icon={AlertTriangle}
            tone="danger"
            hint={`${kpis.nbImpayees} factures en retard / impayées`}
          />
        </div>
        <div className="animate-slide-up stagger-4">
          <KpiCard
            ...
            icon={Users}
            tone="warning"
            hint={`Taux d'impayés ${kpis.tauxImpaye.toFixed(1)} %`}
          />
        </div>
      </div>
