...
import { methodLabels, formatTND, formatDate, clientById, DEFAULT_PER_PAGE } from "@/lib/store";
import type { Payment } from "@/lib/api";

export const Route = createFileRoute("/paiements")({
  beforeLoad: () => {
    requireRoutePermission("paiements:view");
  },
  head: () => ({
    meta: [
...
function PaymentsPage() {
  ...
  const filteredInvoices = useMemo(() => {
    const t = factureSearch.toLowerCase();
    return invoices.filter((i) => {
      const clientName = clientById(i.clientId, clients)?.nom ?? "";
      return (
        i.id.toLowerCase().includes(t) ||
        clientName.toLowerCase().includes(t)
      );
    });
  }, [invoices, factureSearch, clients]);
  ...
  function handleUpdate(data: Omit<Payment, "id">) {
    ...
    updatePayment.mutate({ id: editPayment.id, data });
    setEditPayment(null);
  }
              ...
              <div className="relative">
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    if (e.target.value === "") setFactureId("");
                  }}
                  onFocus={() => setShowSuggestions(true)}
                  placeholder="Rechercher par référence ou client…"
                  className="mt-1 w-full rounded-lg border border-input bg-background py-2 pl-8 pr-3 text-sm outline-none focus:ring-2 focus:ring-ring/40"
                />
                {showSuggestions && factureSearch.trim() !== "" && (
                  <ul className="absolute z-10 mt-1 max-h-48 w-full overflow-auto rounded-lg border border-border bg-card shadow-card">
                    {filteredInvoices.length === 0 && (
                      <li className="px-3 py-2 text-xs text-muted-foreground">Aucune facture trouvée</li>
                    )}
                    {filteredInvoices.map((invoice) => {
                      const reste = invoice.montant - invoice.montantPaye;
                      return (
                        <li
                          key={invoice.id}
                          className="cursor-pointer px-3 py-2 text-sm hover:bg-secondary/60"
                          onClick={() => {
                            setFactureId(invoice.id);
                            setFactureSearch(`${invoice.id} — ${clientById(invoice.clientId, clients)?.nom ?? "Client inconnu"} (${formatTND(reste)} restant)`);
                            setShowSuggestions(false);
                          }}
                        >
                          <span className="font-medium">{invoice.id}</span>
                          <span className="ml-2 text-muted-foreground">
                            {clientById(invoice.clientId, clients)?.nom ?? "Client inconnu"}
                          </span>
                          <span className="float-right text-xs text-muted-foreground">
                            {formatTND(reste)} restant
                          </span>
                        </li>
                      );
                    })}
