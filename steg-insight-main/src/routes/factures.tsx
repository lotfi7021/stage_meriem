...
export const Route = createFileRoute("/factures")({
  beforeLoad: () => {
    requireRoutePermission("factures:view");
  },
  head: () => ({
    meta: [
...
function InvoicesPage() {
  ...
  async function handleCreate(data: Omit<Invoice, "id">) {
    await createInvoice.mutateAsync(data);
    setShowForm(false);
    toast.success("Facture créée", { description: "La facture a été ajoutée." });
  }

  async function handleUpdate(data: Omit<Invoice, "id">) {
              ...
              {(
                ...
                >
                  <span className="inline-flex items-center gap-1">
                    {label}
                    {key && (
                      <SortIcon column={key as SortKey} sortKey={sortKey} sortDir={sortDir} />
                    )}
                  </span>
                </th>
              ))}
